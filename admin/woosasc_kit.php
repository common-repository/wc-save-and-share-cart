...
if ( ! class_exists( 'woosasc_kit' ) ) {
	class woosasc_kit {
		...
		function admin_menu() {
			add_submenu_page( 'edit.php?post_type=woosasc_cart', esc_html__( 'Essential Kit', 'wc-save-and-share-cart' ), esc_html__( 'Essential Kit', 'wc-save-and-share-cart' ), 'manage_options', 'woosasc_kit', array(
				$this,
				'settings_page'
			) );
		}
		...
		function settings_page() {
            ...
            <div class="wrap">
                ...
                <div class="wp-list-table widefat plugin-install-network">
					...
					$updated      = false;
					$plugins_info = get_site_transient( 'woosasc_kit_plugins_info' );
											...
											<?php if ( $this->is_plugin_installed( $_plugin ) ) {
												if ( $this->is_plugin_active( $_plugin ) ) {
                                                    ...
                                                    <a href="<?php echo esc_url( $this->deactivate_plugin_link( $_plugin ) ); ?>"
                                                       class="button deactivate-now">
														<?php esc_html_e( 'Deactivate', 'wc-save-and-share-cart' ); ?>
                                                    </a>
													<?php
												} else {
													?>
                                                    <a href="<?php echo esc_url( $this->activate_plugin_link( $_plugin ) ); ?>"
                                                       class="button activate-now">
														<?php esc_html_e( 'Activate', 'wc-save-and-share-cart' ); ?>
                                                    </a>
													<?php
												}
											} else { ?>
                                                <a href="<?php echo esc_url( $this->install_plugin_link( $_plugin ) ); ?>"
                                                   class="install-now button wpckit-install-now">
													<?php esc_html_e( 'Install Now', 'wc-save-and-share-cart' ); ?>
                                                </a>
											<?php } ?>
                                        </li>
                                        <li>
                                            <a href="<?php echo esc_url( $details_link ); ?>"
                                               ...
                                               aria-label="<?php echo esc_attr( sprintf( esc_html__( 'More information about %s', 'wc-save-and-share-cart' ), $plugin_info['name'] ) ); ?>"
                                               data-title="<?php echo esc_attr( $plugin_info['name'] ); ?>">
												<?php esc_html_e( 'More Details', 'wc-save-and-share-cart' ); ?>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
							...
							<?php 
								...
								if ( isset( $plugin_info['version'] ) ) { ?>
                                    <div class="column-updated">
										<?php echo esc_html__( 'Version', 'wc-save-and-share-cart' ) . ' ' . $plugin_info['version']; ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['active_installs'] ) ) { ?>
                                    <div class="column-downloaded">
										<?php echo number_format_i18n( $plugin_info['active_installs'] ) . esc_html__( '+ Active Installations', 'wc-save-and-share-cart' ); ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['last_updated'] ) ) { ?>
                                    <div class="column-compatibility">
                                        <strong><?php esc_html_e( 'Last Updated:', 'wc-save-and-share-cart' ); ?></strong>
                                        <span><?php printf( esc_html__( '%s ago', 'wc-save-and-share-cart' ), esc_html( human_time_diff( strtotime( $plugin_info['last_updated'] ) ) ) ); ?></span>
                                    </div>
								<?php }
					...
					if ( $updated ) {
						set_site_transient( 'woosasc_kit_plugins_info', $plugins_info, 24 * HOUR_IN_SECONDS );
					}
					?>
                </div>
            </div>
		...
		public function activate_plugin_link( $plugin) {
			...
			return wp_nonce_url( admin_url( 'admin.php?page=woosasc_kit&action=activate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'activate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
			
		}

		public function deactivate_plugin_link( $plugin ) {
			return wp_nonce_url( admin_url( 'admin.php?page=woosasc_kit&action=deactivate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'deactivate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
		}
	}
